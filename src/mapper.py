...
class Parser:
    ...
    def parse_mapping(self) -> Dict:
        ...
        return {
            'name': name,
            'source': source or {'type': 'CSV', 'config': {}},
    ...
    def parse_source_target(self) -> Dict:
        ...
        if not type_token:
            raise SyntaxError("Expected source/target type (XML|CSV|DB|EDI)")
        
        if type_token[0] in ('XML', 'CSV', 'DB', 'EDI'):
        ...
        return {'type': config_type, 'config': config}
...
class CodeGenerator:
    ...
    def generate(self) -> str:
        ...
        if source_type == 'XML' or target_type == 'XML':
        ...
        # Source handlers
        lines.extend(self._generate_source_handlers(source_type))
        
        # Target handlers
        lines.extend(self._generate_target_handlers(target_type))
        
        # Main function
        lines.extend(self._generate_main_function(source_type, target_type))
    ...
    def _generate_main_function(self, source_type: str, target_type: str) -> List[str]:
        """Generate main execute_mapping function."""
        lines = []
        
        source_config = self.mapping['source']['config']
